...
class serCommand extends ContainerAwareCommand
{
    ...
    protected function products($crawler, $extra = null) {
        if( ($crawler->filter('.productList-item .productTitle')->count()) and 
            ...
            foreach ($filter as $i => $content) {
                ...
                if (
                    ($this->subcategory_count == 32) or //an�lisis del agua
                    ($this->subcategory_count == 33) or //iluminaci�n
                    ($this->subcategory_count == 34) or //calefacci�n
    ...
    protected function product($crawler) {
        ...
        if($composition) {
            $body .= $join . "<h3>Composici�n:</h3>";
            $body .= $join . $composition;
        }
        if($qualityanalysis) {
            $body .= $join . "<h3>An�lisis de Calidad:</h3>";
