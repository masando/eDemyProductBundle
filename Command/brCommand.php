...
class BarbaraCommand extends ContainerAwareCommand
{
    private $client;
    private $domain;
    private $categoria;
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->categorias($crawler, 'table.box_width_left li.bg_list');
    }
        ...
        if (iterator_count($filter)) {
            foreach ($filter as $i => $content) {
                ...
                if($this->getCategoryProducts) {
                    ...
                    $crawler = $this->follow($crawler->filter('a')->attr('href'));
                    $this->subcategorias($crawler, 'li.bg_list_sub');
                }
    ...
    protected function writeCsv($products, $file = 'products.csv') {
        $fp = fopen('/var/__/web/d/' . $this->sluggify($file) . '.csv', 'w');
        fputcsv($fp, array(
            'Handle', 
            'Title',
            'Body (HTML)',
